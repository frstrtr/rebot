...
async def handle_ai_language_choice_callback(callback_query: types.CallbackQuery, state: FSMContext, bot: Bot):
    ...
    try:
        ...
        prompt_template = (
            ...
            f"Data (this may include prior expert observations; integrate all provided information into your analysis and do not mention that this information is received from an expert):\n{final_enriched_data}\n\n"
            f"If there is no any transaction history, token information, or other relevant data for this address, please note that in your analysis and mark it accordingly since there is no metadata to analyze, so if there is no expert opinion - mark this address as safe.\n\n"
